...
var d = new Array(10);
//保存大DIV当前装的小DIV的编号
var d_direct=new Array(     //可以移动的方向！！！的位置！！！标号的位置
...
//保存大DIV编号的可移动位置编号
var d_posXY=new Array(      //自己的位置，坐标！！！！
        [0],//同样，我们不使用第一个元素
...
//大DIV编号的位置
//大的div一共有九个，数组的下表是？  数组内容是？
d[1]=1;d[2]=2;d[3]=3;d[4]=4;d[5]=5;d[6]=6;d[7]=7;d[8]=8;d[9]=0;
//默认按照顺序排好，大DIV第九块没有，所以为0，我们用0表示空白块

function move(id){     //id表示要移动的方块的标号，因此断定 d中数字的大小是真正的方块（带数字，能渲染）下标是固定的，表示的是固定的位置
    //移动函数
    var i=1;
    for(i=1; i<10; ++i){
        if( d[i] == id )
    ...
    target_d=whereCanTo(i);       //这里返回的是相当于可以移动的位置的下标！！！
    //用来找出小DIV可以去的位置，如果返回0，表示不能移动，如果可以移动，则返回可以去的位置编号
    if( target_d != 0){
        d[i]=0;                   //执行逻辑，当前置为0，目标位置变为此位置（看不到的逻辑），把现在的样式提供给目标位置（看不到的）
        //把当前的大DIV编号设置为0，因为当前小DIV已经移走了，所以当前大DIV就没有装小DIV了
        d[target_d]=id;
        //把目标大DIV设置为被点击的小DIV的编号
        document.getElementById("d"+id).style.left=d_posXY[target_d][0]+"px";  //这些大的div的位置都是不变的
        document.getElementById("d"+id).style.top=d_posXY[target_d][1]+"px";
        //最后设置被点击的小DIV的位置，把它移到目标大DIV的位置
    }
    //如果target_d不为0，则表示可以移动，且target_d就是小DIV要去的大DIV的位置编号
    var finish_flag=true;    //每move一次都会判断是否完成游戏！！！
    //设置游戏是否完成标志，true表示完成
    for(var k=1; k<9; ++k){
        if( d[k] != k){
            finish_flag=false;
...
function whereCanTo(cur_div){
    ...
    for(j=0; j<d_direct[cur_div].length; ++j){
        //把所有可能去的位置循环遍历一下
        if( d[ d_direct[cur_div][j] ] == 0 ){    //d_direct[cur_div][j]存储的相当于大div的下标
            move_flag=true;
            break;                               //有且仅有一个位置可以动！因此找到一个之后就可以退出for循环了
        }
        //如果目标的值为0，说明目标位置没有装小DIV，则可以移动，跳出循环
    }
    if(move_flag == true){
        return d_direct[cur_div][j];            //返回可以动的大div的下标
    }else{
        return 0;
    }
...
function timer(){
    time+=1;//一秒钟加一，单位是秒
    var min=parseInt(time/60);//把秒转换为分钟，一分钟60秒，取商就是分钟//向前取整？？？
...
function random_d(){
    for(var i=9; i>1; --i){
        ...
        if(d[i]!=0){    //如果当前要遍历的大div中的小div不为空
...
//reset---
//random_d打乱方块---打乱方块，对调某些方块（将大div的位置赋给小div）并赋值给图标，并在极短的时间内绘制
//---start